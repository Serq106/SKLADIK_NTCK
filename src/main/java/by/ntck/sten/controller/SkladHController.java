...
public class SkladHController {
    ...
    public void history(int id_historyOperation, String Dates, int Id_row, String TableName, String Operation,
            int id_kladovshik, double kol_vo, double kol_vo_old, int id_sklad) {
            ...
            for (SkladH skladH2 : skladH_out) {
                if (kol_vo > 0) {
                    ...
                    if (summa > 0 && skladH2.getKol_vo() != 0) {
                        ...
                        this.history(0, dateFormat.format(new Date()), skladH_.getId(), "SkladH", "out",
                                id_kladovshik, skladH.getKol_vo(), skladH2.getKol_vo(), sklad_id);
                        ...
                        this.history(0, dateFormat.format(new Date()), skladH_.getId(), "SkladH", "out",
                                id_kladovshik, skladH.getKol_vo(), kol_vo, sklad_id);
                        skladH.setKol_vo(kol_vo);
                        double count = skladHService.Count(id_kladovshik, sklad_id);
                        Sklad sklad = skladService.getById(sklad_id);
                        sklad.setKolvo(count);
